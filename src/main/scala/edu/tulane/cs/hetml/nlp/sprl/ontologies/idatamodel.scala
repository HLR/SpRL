...
object idatamodel extends DataModel {
 ...
 // val tokens = node[Token]((t: Token) => t.getId)
  ...
  val tripsPhrases = node[Phrase]((p:Phrase) => p.getId)
  val tripsRelations =node[Relation]((r:Relation) => r.getParent().getId+"_"+r.getId)
//
//
//
//  /*
...
//   */
  ...
  val sentenceToPhrase = edge(sentences, tripsPhrases)
  sentenceToPhrase.addSensor(sentenceToPhraseMatching _)

  val tripsRelationToPhrase1 = edge(tripsRelations, tripsPhrases)
  tripsRelationToPhrase1.addSensor((r:Relation, p:Phrase) => (r.getArgumentId(0) == p.getId))

  val tripsRelationToPhrase2 = edge(tripsRelations, tripsPhrases)
  tripsRelationToPhrase1.addSensor((r:Relation, p:Phrase) => (r.getArgumentId(1) == p.getId))
